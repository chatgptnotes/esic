...
-- 1. Visit Complications Junction Table
CREATE TABLE IF NOT EXISTS visit_complications (
...
CREATE INDEX IF NOT EXISTS idx_visit_complications_visit_id ON visit_complications(visit_id);
CREATE INDEX IF NOT EXISTS idx_visit_complications_complication_id ON visit_complications(complication_id);
...
ALTER TABLE visit_complications ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Allow all operations for authenticated users" ON visit_complications
...
CREATE TRIGGER update_visit_complications_updated_at BEFORE UPDATE ON visit_complications FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
COMMENT ON TABLE visit_complications IS 'Junction table linking visits to complications (many-to-many)';
